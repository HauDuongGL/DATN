...
class RecognitionActivity : BaseActivity<RecognitionActBinding>() {
    ...
    override fun initData() {
        ...
        if (uri != null) {
            ...
            // Try to resolve real path; may be null on newer Android versions
            var realPath = RealPathUtil.getRealPath(this, parsedUri)
            var file = if (realPath != null) File(realPath) else File("")

            // Fallback: copy content from URI to a temp file if real path is invalid
            if (!file.exists() || file.length() == 0L) {
                val mime = contentResolver.getType(parsedUri)
                val suffix = when (mime) {
                    "image/png" -> ".png"
                    "image/webp" -> ".webp"
                    else -> ".jpg"
                }
                val temp = File.createTempFile("upload_", suffix, cacheDir)
            ...
            // Display using URI directly (more resilient)
